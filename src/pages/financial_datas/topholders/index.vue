    ...
    <view>
      ...
      <u-table :th-style="{}" >
        ...
        <u-tr class="tr" v-for="(item,index) in list" :key="index">
          ...
          <u-td><view>{{ item.reportDate }}</view><view>{{ item.biandongbili }}</view></u-td>
        </u-tr>
      ...
      setTimeout(() => {
        this.$u.api.pageTopHolders({
          ...
          this.list.forEach(e => {
            ...
            e.biandongbili = this.$u.lodash.ceil((e.biandongbili / 1), 3)
          })
        })
      }, 200)
    },
